...
SELECT customer_last_name, customer_first_name, customer_id, customer_postal_code FROM customer LIMIT 10;
...
SELECT vendor_id, COUNT ( market_date) AS number_of_times_rented  FROM vendor_booth_assignments
GROUP BY vendor_id;
...
SELECT customer_last_name, customer_first_name, customer_purchases.customer_id, SUM(quantity*cost_to_customer_per_qty) AS transaction_amount
FROM customer_purchases
JOIN customer
ON customer_purchases.customer_id=customer.customer_id
GROUP BY customer_purchases.customer_id
HAVING transaction_amount >= 2000
...
HAVING purchase_year= '2022' AND purchase_month= '04'
